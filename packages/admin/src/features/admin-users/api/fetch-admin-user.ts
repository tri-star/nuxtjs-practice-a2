...
export async function fetchAdminUserAsync(adminUserId: string) {
  const { data, error } = await useAsyncData(`AdminUser-${adminUserId}`, async () => {
    try {
      if (adminUserId.match(/[^0-9A-Za-z]+$/)) {
        throw new Error('無効なIDです')
      }
...
export function clearFetchAdminUserCache(adminUserId: string) {
  clearNuxtData(`AdminUser-${adminUserId}`)
}
